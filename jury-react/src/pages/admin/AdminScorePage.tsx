...
export default function AdminScorePage() {
  ...
  const fetchCriteriaData = useCallback(async () => {
    try {
      ...
      const q = query(collection(db, 'criteria'), orderBy('displayOrder', 'asc'));
      const snap = await getDocs(q);
      
      let total = 0;
      const loadedCriteria: CriterionItem[] = snap.docs.map(doc => {
        const data = doc.data() as any;
        if (data.active) total += (data.maxMarks || 0);
  ...
  const searchTeamsData = useCallback(async () => {
    if (!teamSearch.trim()) {
      setTeams([]);
      return;
    }
    ...
    try {
      const [teamsSnap, venuesSnap, assignmentsSnap, evalSnap] = await Promise.all([
      ...
      const searchStr = teamSearch.trim().toLowerCase();
      
      const filteredTeams = teamsSnap.docs
        .map(doc => ({ id: doc.id, ...doc.data() } as any))
        .filter(t => 
          (t.teamNumber && t.teamNumber.toLowerCase().includes(searchStr)) ||
          (t.teamName && t.teamName.toLowerCase().includes(searchStr))
        );

      const formatted = filteredTeams.map(team => {
        ...
        return {
          ...
          status,
          finalScore,
          venue: { name: venue ? venue.name : 'Unknown' }
        };
      });
    ...
    <div className="space-y-8 pb-12 text-white">
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            <h2 className="text-lg font-bold text-white">Active Rubric Criteria</h2>
      ...
      <div className="space-y-4 pt-6 border-t border-white/[0.08]">
        ...
        {loadingTeams ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-2">
            {teams.map((t) => (
              <div key={t.id} className="bg-[#0e1420] border border-white/[0.08] rounded-2xl p-5 space-y-3">
      ...
      {addModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#0e1420] border border-white/10 rounded-2xl w-full max-w-md p-6 sm:p-7 shadow-2xl">
            <h2 className="text-xl font-bold text-white mb-1">Add Scoring Criterion</h2>
      ...
      {editModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#0e1420] border border-white/10 rounded-2xl w-full max-w-md p-6 sm:p-7 shadow-2xl">
            <h2 className="text-xl font-bold text-white mb-1">Edit Scoring Criterion</h2>

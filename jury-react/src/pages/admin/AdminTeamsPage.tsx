...
export default function AdminTeamsPage() {
  ...
  const fetchTeamsData = useCallback(async () => {
    try {
      ...
      let fetchedTeams = teamsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as any));

      if (search.trim()) {
        const s = search.trim().toLowerCase();
        fetchedTeams = fetchedTeams.filter(t => 
          (t.teamNumber && t.teamNumber.toLowerCase().includes(s)) ||
          (t.teamName && t.teamName.toLowerCase().includes(s))
        );
      }

      const formattedTeams = fetchedTeams.map((team) => {
        const venue = fetchedVenues.find(v => v.id === team.venueId);
        ...
        return {
          ...
          problemStatement: ps ? { id: ps.id, code: ps.code, title: ps.title } : null,
  ...
  }, [search]); // venueId omitted intentionally to avoid loops, it's just for default state
  ...
  const handleAddTeam = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/teams', {
        ...
        body: JSON.stringify({ teamNumber, teamName, venueId }),
      });
      ...
      setError(err.message || 'Failed to create team. Ensure Cloud Functions are running.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleEditTeam = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to update team. Ensure Cloud Functions are running.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-6 pb-12 text-white">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {teams.map((team) => (
            <div
              key={team.id}
              className="bg-[#0e1420] border border-white/[0.08] rounded-2xl p-5 flex flex-col justify-between space-y-4 group hover:border-indigo-500/30 transition-all"
            >
              <div>
      ...
      {addModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#0e1420] border border-white/10 rounded-2xl w-full max-w-md p-6 sm:p-7 shadow-2xl">
            <h2 className="text-xl font-bold text-white mb-1">Add Hackathon Team</h2>
      ...
      {editModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#0e1420] border border-white/10 rounded-2xl w-full max-w-md p-6 sm:p-7 shadow-2xl">
            <h2 className="text-xl font-bold text-white mb-1">Edit Team Information</h2>
